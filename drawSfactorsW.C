...
lat->SetTextFont(42);
  ...
  RooCurve *crv_mc = (RooCurve*) (signal_wjetsregion_mc_fit_before_after->GetListOfPrimitives()->FindObject(Form("doubleExponential_singlemuon_mc%s_Norm[mvamet]",cats[c].c_str())));
  RooCurve *crv_da = (RooCurve*) (signal_wjetsregion_mc_fit_before_after->GetListOfPrimitives()->FindObject(Form("doubleExponential_singlemuon_data%s_Norm[mvamet]",cats[c].c_str())));
  ...
  gr->GetYaxis()->SetTitle("r(E_{T}^{miss}) - W#mu#nu Correction function");
  lat->DrawLatex(0.1,0.92,"#bf{CMS} #it{Preliminary}");
  lat->DrawLatex(0.7,0.94,Form("%s category",cats[c].c_str()));
