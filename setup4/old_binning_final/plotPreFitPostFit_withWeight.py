...
def plotPreFitPostFit(region):
  ...
  for process in processes:
    ...
    for i in range(1,h_prefit[process].GetNbinsX()+1):
      ...
      print "Pre-Fit",process, i, content*width
  ...
  for process in processes:
    ...
    for i in range(1,h_postfit[process].GetNbinsX()+1):
      ...
      print "Pre/Post-Fit",process,i, h_prefit[process].GetBinContent(i), content*width
      if h_prefit[process].GetBinContent(i) > 0. :
        cutstring_p = cutstring_p +  \
            str(h_postfit[process].GetBinContent(i)/ h_prefit[process].GetBinContent(i) ) + \
            "*(met>" + str(h_postfit[process].GetBinLowEdge(i))  + \
            "&&met<="+ str(h_postfit[process].GetBinLowEdge(i+1))+")"
        if i<h_postfit[process].GetNbinsX():
          cutstring_p+="+"
    print process, cutstring_p
  ...
  #c.SaveAs("/afs/cern.ch/user/z/zdemirag/www/Monojet/topupv3/sys_setup3/prefit_postfit_"+region+".pdf")
  #c.SaveAs("/afs/cern.ch/user/z/zdemirag/www/Monojet/topupv3/sys_setup3/prefit_postfit_"+region+".png")
  #c.SaveAs("/afs/cern.ch/user/z/zdemirag/www/Monojet/topupv3/sys_setup3/prefit_postfit_"+region+".C")

  #c.SaveAs("prefit_postfit_"+region+".pdf")
  #c.SaveAs("prefit_postfit_"+region+".png")
  #c.SaveAs("prefit_postfit_"+region+".C")
