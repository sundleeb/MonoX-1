...
void RecoilCorrector::metDistributionType1(double &iMet,double &iMPhi,double iGenPt,double iGenPhi,
  ...
  double p1Cos = - (p1UX*cos(iGenPhi) + p1UY*sin(iGenPhi))/p1U;
...
double RecoilCorrector::getError(double iVal,TF1 *iFit,Recoil iType) {
  ...
  //cout << "====> Error Data : "<<  lEZD2 << " MC : " << lEZM2 << " -- Rat " << lR << " -- DatV " << lZDat << " -- MCV " << lZMC << " -- " << lWMC << " -- Total " << sqrt(lER) << " -- W : " << lEW2 << " -- All : " << lVal << " -- " << sqrt(lVal)<< endl;
  return sqrt(lVal);
}
