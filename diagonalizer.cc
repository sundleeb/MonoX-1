...
class diagonalizer {
  ...
  public:
    ...
    void generateWeightedTemplate(TH1F *, RooFormulaVar *, std::string pvar, RooRealVar &, RooDataSet *);
...
void diagonalizer::generateWeightedTemplate(TH1F *histNew, TH1F *pdf_num, std::string wvar, std::string var, RooDataSet *data){
  ...
  // wvar is the weight, var is for the plot
...
void diagonalizer::generateWeightedTemplate(TH1F *histNew, RooFormulaVar *pdf_num, std::string pvar, RooRealVar &var, RooDataSet *data){
  //pvar is the plotting var, var is the weighintg var
  int nevents = data->numEntries();
  const char *varname = var.GetName();
  for (int ev=0;ev<nevents;ev++){
    const RooArgSet *vw = data->get(ev);
    double val    = vw->getRealValue(varname);
    double pval    = vw->getRealValue(pvar.c_str());
  ...
  histNew->GetXaxis()->SetTitle(pvar.c_str());
}
